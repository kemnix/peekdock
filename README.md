...
1. Download the latest `peekdock_v1.0.0.zip` from the [Releases page](https://github.com/kemnix/peekdock/releases) and unzip it.
...
PeekDock uses macOS ScreenCaptureKit to grab the Zoom window, then immediately crops each frame to the region you selected before saving. Only the cropped region is written to disk — the original frame is discarded from memory right after. Everything runs locally on your Mac; nothing is uploaded.
...
1. [Releases ページ](https://github.com/kemnix/peekdock/releases) から最新の `peekdock_v1.0.0.zip` をダウンロードし、解凍
2. `PeekDock.app` を `アプリケーション` フォルダへ移動
3. **初回起動** — PeekDockはApple公証（notarize）されていないため、Gatekeeperに標準ではブロックされます。以下の方法で起動してください:
...
PeekDockはZoomウィンドウをmacOSのScreenCaptureKit経由で取得し、各フレームを選択した範囲に即時クロップしてから保存します。ディスクに書き込まれるのはクロップ後の範囲のみで、元のフレームはメモリ上で即破棄されます。すべての処理はMac上で完結し、外部には何も送信しません。
